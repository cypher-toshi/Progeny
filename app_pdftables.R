...
TCT_Batch_id_extract <- function(input, output){
  ...
  convert_pdf(input_file = input, output_file = "protocol.csv", api_key = "dt78j62arbh8") 
  output <- read.csv("protocol.csv", sep = ".", header = F, col.names = "Text") %>% 
...
TCT_Pdf_extract <- function(input, output){
  ...
  output <- read.csv("protocol.csv", sep = ".", header = F, col.names = "Text") %>% 
    ...
    separate(Text, into = c("Date", "Time", "Operator", "Process"), sep = ",", extra = "merge") %>%   
...
TCT_log_event <- function(input, input2, output){
  ...
  output <- right_join(input, input2) %>% 
    ...
    mutate(Event_type = ifelse(grepl("Take Sample", Process), "Sampling",
                               ifelse(grepl("Feed", Process), "Matrix",
                                      ifelse(grepl("Activate shaker", Process), "Matrix",
                                             ifelse(grepl("Deactivate shaker", Process), "Matrix",
                                                    ifelse(grepl("Volume reduction", Process), "Matrix",
                                                           ifelse(grepl("Transduction", Process), "Matrix",
                                                                  ifelse(grepl("Spin", Process), "Matrix",
                                                                         ifelse(grepl("Medium Bag", Process), "Matrix",
                                                                                ifelse(grepl("Culture wash", Process), "Matrix",
                                                                                       ifelse(grepl("Media exchange", Process), "Matrix",
                                                                                              ifelse(grepl("Harvest", Process), "Matrix", "Others")))))))))))) %>% 
    mutate(Event_text = ifelse(grepl("Feed", Process), "Feed", 
                               ifelse(grepl("Activate shaker", Process), "Shaker on", 
                                      ifelse(grepl("Deactivate shaker", Process), "Shaker off", 
                                             ifelse(grepl("Volume reduction", Process), "Vol. red.", 
                                                    ifelse(grepl("Transduction", Process), "Transduction", 
                                                           ifelse(grepl("Spin", Process), "Spin", 
                                                                  ifelse(grepl("Medium Bag", Process), "Medium bag exchange", 
                                                                         ifelse(grepl("Culture wash", Process), "Culture wash", 
                                                                                       ifelse(grepl("Media exchange", Process), "Media exchange", 
                                                                                              ifelse(grepl("Harvest", Process), "Harvest", 
                                                                                                     ifelse(grepl("Take Sample", Process), "Sample", Process))))))))))))
...
server <- function(input, output) {
  ...
  observeEvent(input$plotPressureButton, {
    ...
    if (!is.null(file_tct)) {
      ...
      # Establish vertical lines
      vertical.lines <- c(pressure_data %>% filter(Day_event == "yes") %>% distinct(Time_h) %>% pull(Time_h))
      event.lines.m <- c(event_meta_list %>% filter(Event_type == "Matrix") %>%  pull(Time_h))
      event.text.m <- c(event_meta_list %>% filter(Event_type == "Matrix") %>%  pull(Event_text))
      event.lines.s <- c(event_meta_list %>% filter(Event_type == "Sampling") %>%  pull(Time_h))
      event.text.s <- c(event_meta_list %>% filter(Event_type == "Sampling") %>%  pull(Event_text))
